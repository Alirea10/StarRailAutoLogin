...
class Login:
    ...
    def login_bilibili(account, password):
        ...
        if check("res/img/bilibili_account.png", interval=0.2, max_time=20):
            click("res/img/bilibili_account.png")
        time.sleep(1)
        SRAOperator.copy(account)
        SRAOperator.paste()
        time.sleep(1)
        ...
        SRAOperator.copy(password)
        SRAOperator.paste()
        if check("res/img/bilibili_agree2.png"):
            click("res/img/bilibili_agree2.png", -26)
        click("res/img/bilibili_enter_game.png")
        if check("res/img/welcome.png", interval=0.2, max_time=20):
            logger.info("登录成功")
            return 1
        elif check("res/img/bilibili_verify.png"):
...
if __name__ == "__main__":
    ...
    tasks = [
        ...
        (login.start_game,
         (login.game_path, login.game_type, int(login.server), login.enable_login, login.username, login.password)),
        (login.check_game, ()),
    ]
